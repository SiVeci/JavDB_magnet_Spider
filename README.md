...
在 WebUI 的“设置 → 磁力评分优先级”中，可为三级固定分值选择条件。评分条件池固定为“无码资源”“高清资源”“字幕资源”和“文件体积最大”，四项中选择三项且不能重复；分值固定为 100、10、1，默认映射为 100=无码、10=高清、1=字幕。

“文件体积最大”只比较同一影片中的有效磁力候选，并且候选文件大小必须大于 0；最大值并列时同时命中，大小未知或为 0 时不命中该条件。同分候选仍按发布日期倒序、文件大小倒序排列。磁力验活、Dead/Error 的 -200 扣分，以及人工选择行为保持不变。

保存评分规则不会自动重算历史数据。数据库集合页的“自动选择”会按当前已保存规则重评作用范围内的全部历史候选并重新选择推荐磁力，且会覆盖目标影片此前的人工选择；已勾选集合时只处理已勾选集合，未勾选时处理当前集合页中的全部集合。旧候选只根据文件名回推无码、高清、字幕条件，不联网补抓；文件名无法恢复的历史网页标签可能无法命中。新任务仍会在启动时快照当前规则，最大文件、并列最大、未知大小、验活和 Dead/Error 的 -200 规则保持不变。

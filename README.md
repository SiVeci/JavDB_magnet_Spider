...
- **排行榜集成**：全面支持浏览与提取 JavDB 排行榜数据（包含日周月常规榜单、热播榜单及支持动态年份/分类的 TOP250 榜单），支持一键将排行榜下发为爬取任务，并在榜单页无缝集成磁力检测与多条件标签过滤。
...
- **标签解析与动态过滤**：爬取时自动提取并保存每部影片的“类别/标签”信息。WebUI 支持基于标签对普通的抓取合集以及各类排行榜进行多条件组合（包含/排除）筛选，实现按需定制 CSV 导出与一键复制。

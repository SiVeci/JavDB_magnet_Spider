...
function renderCollectionToolbarActions(collectionName) {
    const item = collectionItem(collectionName) || { name: collectionName, count: 0, tags: [], time: '' };
    const jsName = escapeJs(collectionName);
    const incrementalButton = item.has_source_url
        ? `<button onclick="enqueueCollectionIncremental('${jsName}', event)" title="增量爬取此集合" aria-label="增量爬取此集合" class="btn btn-icon-sm btn-info text-xs">⟳</button>`
        : `<button type="button" disabled title="缺少原始 URL，无法快捷增量" aria-label="缺少原始 URL，无法快捷增量" class="btn btn-icon-sm btn-soft text-xs">⟳</button>`;
    return `
        <div class="ml-auto flex shrink-0 items-center gap-1">
            <button onclick="copyMagnets('${jsName}')" title="复制集合磁力" aria-label="复制集合磁力" class="btn btn-icon-sm btn-info text-xs">⧉</button>
            <button onclick="downloadCsv('${jsName}')" title="下载 CSV" aria-label="下载 CSV" class="btn btn-icon-sm btn-success text-xs">⇩</button>
            ${incrementalButton}
            <button onclick="deleteFiles(['${jsName}'])" title="删除集合" aria-label="删除集合" class="btn btn-icon-sm btn-danger">
...
function renderCollectionBody(collectionName) {
    const toolbar = document.getElementById(`collection-toolbar-${collectionName}`);
    ...
    if (toolbar) toolbar.innerHTML = renderCollectionToolbarHeader(collectionName, filtered);
    body.innerHTML = renderCollectionFilter(collectionName, data.available_tags || [], filtered.length, data.movies.length)
        + renderMovies(collectionName, filtered);
    fitMovieTags(body);
}

function renderCollectionFilter(collectionName, availableTags, filteredCount, totalCount) {
    const selected = selectedCollectionTags(collectionName);
    const excluded = selectedExcludeTags(collectionName);
    ...
    return `
        <div class="mb-3 flex shrink-0 flex-wrap items-center gap-2">
            <div class="relative min-w-0" data-menu-root="tag-filter">
                <button type="button" onclick="toggleCollectionTagDropdown('${escapeJs(collectionName)}', event)" class="flex h-7 min-w-[104px] items-center justify-between gap-2 rounded border border-[color:var(--c-border)] bg-surface px-2 text-left text-xs font-bold text-[color:var(--c-neutral-text)] transition-colors hover:bg-[color:var(--c-surface-sunken)]">
            ...
            <div class="relative shrink-0" data-menu-root="exclude-filter">
                <button type="button" onclick="toggleExcludeDropdown('${escapeJs(collectionName)}', event)" class="flex h-7 min-w-[68px] items-center justify-between gap-1 rounded border px-2 text-left text-xs font-bold transition-colors ${excluded.length ? 'border-[color:var(--c-danger)] bg-danger-soft text-danger-text' : 'border-[color:var(--c-border)] bg-surface text-[color:var(--c-text-muted)] hover:bg-[color:var(--c-surface-sunken)]'}">
            ...
            ${renderCollectionToolbarActions(collectionName)}
        </div>`;
}
...
async function downloadCsv(name) {
    try {
        await apiDownloadBlob(`/api/download?name=${encodeURIComponent(name)}${tagsQuery(name)}`, name);
    } catch (err) {
...
async function copyMagnets(name) {
    await copyMagnetsFromUrl(`/api/magnets?name=${encodeURIComponent(name)}${tagsQuery(name)}`);
}
